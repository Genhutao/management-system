...
func main() {
	...
	if err != nil {
		log.Printf("[Warn] Casbin init warning: %v, fallback to role-based check", err)
	}
					...
					students := authenticated.Group("/students")
					{
						...
						students.DELETE("/clear", studentCtrl.ClearAllStudents)
							...
							deductions := authenticated.Group("/deductions")
							{
								...
								deductions.GET("/morning-dorm-reports", deductionCtrl.GetMorningDormReports) // 宿管上午上报数据(楼层/宿管名/照片/文本)
								deductions.GET("/export-csv", deductionCtrl.ExportDeductionsCSV)
								deductions.DELETE("/:id", deductionCtrl.DeleteDeduction)
							}
